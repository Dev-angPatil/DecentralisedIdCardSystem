import { connectWallet as solConnect } from "./blockchain.js";
...
export async function connectWallet() {
  try {
    ...
    addNotification("Wallet connected", "Phantom wallet connected successfully.");
    renderSharedElements();
    showToast("Wallet connected", result.publicKey, "success");
  } catch (err) {
    showToast("Wallet error", err.message, "failed");
  }
}
  addNotification(
    "Wallet connected",
    "Mock Phantom wallet is ready for future signing."
  );

  // IMPORTANT: update UI in correct order
  renderHeader();
  updateWalletCopy();
...
function bindGlobalActions() {
  const button = document.querySelector("[data-connect-wallet]");
  if (!button) return;

  button.onclick = null; // clear old
